...
export async function approveEscrow(owner: Address, amount: bigint): Promise<Hex> {
  ...
  if (receipt.status !== 'success') throw new Error('pUSDC approval transaction failed.');
  return hash;
}
...
export async function fundContribution(input: FundContributionInput): Promise<Hex> {
  ...
  if (input.acceptanceDeadline <= now) {
    throw new Error('Acceptance deadline must be in the future.');
  }
  if (input.deliveryDeadline <= input.acceptanceDeadline) {
    throw new Error('Delivery deadline must be after the acceptance deadline.');
  }
  ...
  const hash = await walletClient.writeContract(request);
  const receipt = await publicClient.waitForTransactionReceipt({ hash });
  if (receipt.status !== 'success') throw new Error('Escrow funding transaction failed.');
  return hash;
}
